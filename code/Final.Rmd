...
# Times Series
## Exploration donnees brutes et decomposition 

$$y{t} = S{t} + T{t} + R{t}$$
```{r}
ts_ggplot(cbind(as.ts(tabac[,2]), as.ts(sante[,6])))
```

## Decomposition : Focus sur le Trend cycle 
...
inputPanel(
  ...
  shiny::selectInput("variable", "Variable", # lorsque les time series sont multivaries
...
```{r}
tabac2 <- apply.monthly(tabac[,2],FUN=mean)
age2 <- apply.monthly(age[,2],FUN=mean)

ma_tabac2 <- ma(tabac2, 5)
plot(ma_tabac2)
ma_age2 <- ma(age2, 4)
plot(ma_age2)

autoplot(cbind(ma_tabac2, ma_age2))

bind <- cbind(tabac[,2], age[,2])
bind[,1]
autoplot(bind, facets=FALSE) +
  autolayer(ma(bind[,1:2], 4), series="5-MA")
```
...
# ```
      ...
      autoplot(ts(theme1())) +
        autolayer(ma(theme1(), 5), series="5-MA") +
        xlab("Années") + ylab("Nombre de paquets") +
        ggtitle("Nombre moyen de paquets consommes par personne")
