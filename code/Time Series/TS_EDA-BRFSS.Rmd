...
## Constitution time series sur les moyennes 
...
```{r}
    ...
    tax_rate <- xts(tax_rate_m, order.by=tax_dates) %>% to.monthly()
    tax_revenue <- xts(tax_revenue_m, order.by=tax_dates) %>% to.monthly()
    pack_conso <- xts(pack_conso_m, order.by=tax_dates) %>% to.monthly()
    pack_cost <- xts(pack_cost_m, order.by=tax_dates) %>% to.monthly()
    age <- xts(age_m, order.by=brfss_dates) %>% to.monthly()
    famille <- xts(famille_m, order.by=brfss_dates) %>% to.monthly()
    education <- xts(education_m, order.by=brfss_dates) %>% to.monthly()
    finances <- xts(finances_m, order.by=brfss_dates) %>% to.monthly()
    tabac <- xts(tabac_m, order.by=brfss_dates) %>% to.monthly()
    sante <- xts(sante_m, order.by=brfss_dates) %>% to.monthly()
```
...
```{r}
  autoplot(tax_rate, facets = FALSE) + # cyclic, no trend
    xlab("Annees") + ylab("Pourcentage de taxe par rapport au retail price") +
    ggtitle("Evolution du pourcentage de taxe par rapport au retail price")
  gglagplot(tax_rate) 
...
```{r}
  autoplot(age, facets = TRUE) # upward trend
  gglagplot(age) 
