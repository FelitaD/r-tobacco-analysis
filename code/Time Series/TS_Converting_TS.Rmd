...
- Aggregation par jour et conversion object Date
```{r}
...
- Remplacement des dates manquantes par interpolation lineaire pour faire la conversion en ts
```{r}
times.init <- as.Date(brfss_m$IDATE, format = "%Y-%m-%d")
date_complete <- seq(min(times.init), max(times.init), "day")

brfss_i <- merge(brfss_m[-1], date_complete)  

new_df <-xts(brfss_m[,-1], times.init)

interpol <- na.approx(new_df, xout=seq(min(times.init), max(times.init), "day"))
ts.plot(interpol)

ts_interpol <- ts_ts(interpol)
ts_ggplot(ts_interpol, title="Evolution de la santé mentale des répondants", subtitle="Fumeurs et non fumeurs") +
  theme_tsbox() +
  scale_color_tsbox() # ts_ggplot marche aussi avec xts

brfss_m$IDATE <- as.Date(brfss_m$IDATE, format = "%Y-%m-%d")

```
 - Division par theme
```{r}
    age_m <- brfss_m[, c("IDATE","X_AGEG5YR", "X_AGE65YR", "X_AGE_G")]
    famille_m <- brfss_m[, c("IDATE","MARITAL", "PREGNANT", "NUMADULT", "CHILDREN", "X_CHLDCNT")]
    education_m <- brfss_m[, c("IDATE","EDUCA", "X_EDUCAG", "EMPLOY1")]
    finances_m <- brfss_m[, c("IDATE","INCOME2", "X_INCOMG", "RENTHOM1")]
    tabac_m <- brfss_m[, c("IDATE","SMOKE100", "SMOKDAY2", "STOPSMK2", "LASTSMK2", "USENOW3", "X_SMOKER3", "X_RFSMOK3")]
    sante_m <- brfss_m[, c("IDATE","POORHLTH", "MENTHLTH", "QLMENTL2", "QLSTRES2", "ADDEPEV2", "EMTSUPRT", "LSATISFY")] 
```
