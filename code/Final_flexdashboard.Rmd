...
Choix de variables stationnaires
===================================== 
...
### Test diff : Unit Root tests
```{r}
tabac$SMOKE100 %>% ur.kpss() %>% summary()
tabac$SMOKE100 %>% diff() %>% ur.kpss() %>% summary()

```

### Stabilisation moyenne (diff) 
#### Ordre de differenciation
```{r}
theme_1 <- reactive({ get(input$theme1) })
theme_var_1 <- reactive({ get(input$theme1)[,input$variable1] })

output$console12 <- renderPrint({
  ndiffs(tabac[, "SMOKE100"])
  })
verbatimTextOutput("consol12")
```
...
output$plotly1 <- renderPlotly({
  ...
  if (input$variable1 == "SMOKE100") {
    dat <- apply.quarterly(tabac[,input$variable1], FUN=mean)
    d <- diff(dat)
    ts_ggplot(d)
  } else {
...
### Test 
```{r eval = FALSE}
tabac[, "SMOKE100"] %>% log() %>% nsdiffs()
#> [1] 1
tabac[, "SMOKE100"] %>% log() %>% diff(lag=12) %>% ndiffs()
#> [1] 1
```
...
### Lag plot
```{r}
  d <- diff(apply.quarterly(education$EMPLOY1, FUN=mean))
  gglagplot(d) 
```

### Autocorrelation
```{r}
  d <- diff(apply.quarterly(education$EMPLOY1, FUN=mean), diff=2)
  ggAcf(d)
```

### Comparaison des tendances entre 2 series

```{r}
output$plot3 <- renderPlot({

  ma1 <- ma(diff(apply.monthly(theme_var_1(), FUN=mean)), 5)
  ma2 <- ma(apply.monthly(theme_var_2(), FUN=mean), 5)

  autoplot(cbind(ma1, ma2))
})
...
Regression dynamique {data-orientation=rows}
=====================================  
