...
new_df <-xts(brfss_m[, c("SMOKE100","MARITAL","X_CHLDCNT", "X_AGE_G","SEX","X_EDUCAG","EMPLOY1", "X_INCOMG", "POORHLTH","MENTHLTH","QLMENTL2","QLSTRES2","ADDEPEV2","EMTSUPRT","LSATISFY","USENOW3")],times.init)
...
output$console42 <- renderPrint({
  ...
  fit_sante_mentale <- auto.arima(brfss_interpol[, "SMOKE100"], xreg = xreg_sante_mentale)
  coefficients(fit_sante_mentale)
})
...
output$console45 <- renderPrint({
  xreg_sante_mentale <- cbind(POORHLTH = as.numeric(brfss_interpol[, "POORHLTH"]),
            ...
            LSATISFY = as.numeric(brfss_interpol[, "LSATISFY"]))
  fit_sante_mentale <- auto.arima(brfss_interpol[, "SMOKE100"], xreg = xreg_sante_mentale)
  coefficients(fit_sante_mentale)[3]
  })
...
### Prediction 
#### Taux de personnes ayant fume au  moins 100 cigarettes dans leur vie en fonction de la santé mentale
```{r}
output$plot999 <- renderPlot({
  
  fit <- auto.arima(brfss_interpol[, "SMOKE100"], xreg = brfss_interpol[, "MENTHLTH"], stationary = TRUE)
